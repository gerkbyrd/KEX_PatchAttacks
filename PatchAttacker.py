...
class PatchAttacker:
    def __init__(self, model, mean, std, image_size=244,epsilon=1,steps=500,step_size=0.05,patch_size=31,random_start=True, n_patches=1, pw=None, pl = None):
    ...
    def perturb(self, inputs, labels, loc=None,random_count=1):
        ...
        for _ in range(random_count):
            ...
            idx_init = torch.arange(inputs.shape[0])[:, None]
            ...
            #wind, lind = 2*torch.le(w_idx, 0.5*inputs.shape[2]) - 1, 2*torch.le(l_idx, 0.5*inputs.shape[3]) - 1
            #print(inputs.shape)
            w_idx2= inputs.shape[2] - self.patch_w - 1 - w_idx
            l_idx2= inputs.shape[3] - self.patch_l - 1 - l_idx
            #print(w_idx)
            #print(w_idx2)
            #print(l_idx)
            #print(l_idx2)
            ...
            idx = torch.cat([idx_init,zero_idx, w_idx, l_idx], dim=1)
            idx_list = [idx]

            idx2 = torch.cat([idx_init,zero_idx, w_idx2, l_idx2], dim=1)
            idx_list2 = [idx2]
            for w in range(self.patch_w):
                for l in range(self.patch_l):
                    idx_list.append(idx + torch.tensor([0,0,w,l]))
                    idx_list2.append(idx2 + torch.tensor([0,0,w,l]))
            idx_list = torch.cat(idx_list, dim =0)
            idx_list2 = torch.cat(idx_list2, dim =0)
            # create mask
            mask = torch.zeros([inputs.shape[0], 1, inputs.shape[2], inputs.shape[3]],
                               dtype=torch.bool).cuda()
            mask[idx_list[:,0],idx_list[:,1],idx_list[:,2],idx_list[:,3]] = True
            mask[idx_list2[:,0],idx_list2[:,1],idx_list2[:,2],idx_list2[:,3]] = self.n_patches>=2#2 patch windows
            mask[idx_list[:,0],idx_list[:,1],idx_list[:,2],idx_list2[:,3]] = self.n_patches>=3#3 patch windows
            mask[idx_list[:,0],idx_list[:,1],idx_list2[:,2],idx_list[:,3]] = self.n_patches>=4#4 patch windows
