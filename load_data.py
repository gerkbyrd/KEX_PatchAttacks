...
class PatchTransformer(nn.Module):
    ...
    def __init__(self, size_fix=False, n_patches=1, sub_patches=0):
        ...
        self.sub_patches=int(sub_patches)
        '''
    ...
    def forward(self, adv_patch, lab_batch, img_size, do_rotate=True, rand_loc=True):
        ...
        target_size = torch.sqrt(((lab_batch_scaled[:, :, 3]) ** 2) + ((lab_batch_scaled[:, :, 4]) ** 2)).mul(1/5)
        #target_size = torch.max(target_size, 130.0)
        target_x = lab_batch[:, :, 1].view(np.prod(batch_size))
        target_y = lab_batch[:, :, 2].view(np.prod(batch_size))
        targetoff_x = lab_batch[:, :, 3].view(np.prod(batch_size))
        targetoff_y = lab_batch[:, :, 4].view(np.prod(batch_size))
        if(rand_loc):
            off_x = targetoff_x*(torch.cuda.FloatTensor(targetoff_x.size()).uniform_(-mdelta, mdelta))
            target_x = target_x + off_x
            off_y = targetoff_y*(torch.cuda.FloatTensor(targetoff_y.size()).uniform_(-mdelta ,mdelta))
            target_y = target_y + off_y
        target_y = target_y# - 0.05
        scale = target_size / current_patch_size#*(2**.5)
        if not self.size_fix:
            scale=scale/(self.n_patches**0.5)
        #print(target_size)
        #print(scale)
        #input('from within!')
        scale = scale.view(anglesize)
        #if self.size_fix:
        #    scale = torch.cuda.FloatTensor(anglesize).fill_(1)
        ...
        txx, tyy = [], []
        if self.n_patches==1:
            txx.append((-target_x+0.5)*2)
            tyy.append((-target_y+0.5)*2)
        elif self.n_patches>=2:
            if self.sub_patches!=2:
                txx.append((-target_x-mdelta*targetoff_x+0.5)*2)
                tyy.append((-target_y-mdelta*targetoff_y+0.5)*2)
            if self.sub_patches!=1:
                txx.append((-target_x+mdelta*targetoff_x+0.5)*2)
                tyy.append((-target_y+mdelta*targetoff_y+0.5)*2)
            if self.n_patches >=3:
                txx.append((-target_x+mdelta*targetoff_x+0.5)*2)
        ...
        for tx, ty in zip(txx,tyy):
            ...
            grid = F.affine_grid(theta, b_sh)
            #adv_batch_t = F.grid_sample(adv_batch, grid)
            #msk_batch_t = F.grid_sample(msk_batch, grid)
            patches.append(F.grid_sample(adv_batch, grid))
            masks.append(F.grid_sample(msk_batch, grid))
        ...
        for i, (patch, mask) in enumerate(zip(patches, masks)):
            if i==0:
                adv_batch_t = patch#torch.cuda.FloatTensor(patch.shape).fill_(0)#patches[0]
                msk_batch_t = mask#torch.cuda.FloatTensor(mask.shape).fill_(0)#patches[0]#mask
            else:
                adv_batch_t = adv_batch_t + patch
                msk_batch_t = msk_batch_t + mask
        adv_batch_t = adv_batch_t.view(s[0], s[1], s[2], s[3], s[4])
        msk_batch_t = msk_batch_t.view(s[0], s[1], s[2], s[3], s[4])

        #adv_batch_t = adv_batch.view(s[0], s[1], s[2], s[3], s[4])
        #msk_batch_t = msk_batch.view(s[0], s[1], s[2], s[3], s[4])
        ...
        #exit()
        """
        ret=adv_batch_t * msk_batch_t
        img = ret[0, 0, :, :, :].detach().cpu()
        img = transforms.ToPILImage()(img)
        img.show()
        input('INSIGHTFUL!')
        print(ret.shape)
...
class PatchApplier(nn.Module):
    ...
    def forward(self, img_batch, adv_batch):
        ...
        for adv in advs:
            ...
            #adv_np=adv[0].detach().cpu().numpy()
            #img_np=img_batch[0].detach().cpu().numpy()
            #adv_2d=np.sum(adv_np,axis=0)
            #img_2d=np.sum(img_np,axis=0)
            #with open('arnold' + '.txt', 'a') as f:
                #f.write(str(np.sum(adv_2d!=0)**0.5) + ',\n')
        return img_batch
